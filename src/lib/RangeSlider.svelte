...
<div class="w-full">
  {#if label}
    ...
    >
      ...
      {#if showValue}
        <span
          class="ml-2 text-primary-link font-semibold"
          id={valueId}
          aria-live="polite"
        >
          {value}
  ...
  <div class="relative">
    <input
      ...
      aria-label={ariaLabel ||
        (label
          ? undefined
          : `Slider with minimum ${min}, maximum ${max}, current value ${value}`)}
      aria-labelledby={ariaLabelledBy ||
        (label ? `${uniqueId}-label` : undefined)}
  ...
  <div id={descriptionId} class="sr-only">
    Range slider. Use arrow keys to adjust value. Home and End keys go to
    minimum and maximum values. Page Up and Page Down keys adjust by larger
    increments.
  </div>
</div>
