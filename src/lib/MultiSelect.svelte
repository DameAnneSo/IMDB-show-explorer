...
const selectOption = (option, index = -1) => {
  if (!selectedValues.includes(option)) {
    ...
    announceToScreenReader(
      `${option} selected. ${selectedValues.length} items selected.`
    );
  }
};
...
const removeOption = (option) => {
  ...
  announceToScreenReader(
    `${option} removed. ${selectedValues.length} items selected.`
  );
};
...
<div class="relative w-full" bind:this={containerElement} id={comboboxId}>
  ...
  <div
    ...
    aria-label={label ||
      `MultiSelect with ${selectedValues.length} items selected`}
  ...
  >
    ...
    <div class="ml-2 flex-shrink-0">
      <svg
        class="w-4 h-4 text-neutral-400 transition-transform {isOpen
          ? 'rotate-180'
          : ''}"
        fill="none"
        stroke="currentColor"
    ...
    >
      ...
      {#if !inputValue && options.length > selectedValues.length}
        <div
          ...
          tabindex="-1"
        >
          <span aria-hidden="true">✓</span> Select All ({options.length -
            selectedValues.length} remaining)
        </div>
      {/if}
      ...
      {#if filteredOptions.length === 0 && inputValue}
        <div
          class="px-4 py-2 text-neutral-500 italic"
          role="status"
          aria-live="polite"
        >
