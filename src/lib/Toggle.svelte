...
<div class="flex items-center">
  {#if label}
    <label class="text-sm font-medium text-neutral-700 mr-3" for={toggleId}>
      {label}
    </label>
  {/if}
  ...
  <button
    type="button"
    class="relative inline-flex flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 h-6 w-11 {checked
      ? 'bg-primary-600'
      : 'bg-neutral-200'} {disabled ? 'opacity-50 cursor-not-allowed' : ''}"
    role="switch"
