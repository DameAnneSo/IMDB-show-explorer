<script>
  ...
  const ticks = $derived(
    xScale.ticks(numberOfTicks(xMax - xMin)).filter((tick) => Number.isInteger(tick)),
  );
</script>
...
<g transform={`translate(0 ${height - margins.marginTop - margins.marginBottom})`}>
  ...
  {#each ticks as tick (tick)}
    <g transform={`translate(${xScale(tick)} 0)`}>
      ...
      <text class="axis__tick-label" y={10} dy="0.7em" text-anchor="middle" fill="#000000">
        {tick}
      </text>
    </g>
  {/each}

  <text class="axis__label" x={xScale.range()[1] / 2} text-anchor="middle" y={37}>
    episode number
  </text>
</g>
